...
from research_and_analyst.caching import SearchResultCache
from research_and_analyst.security import get_redactor
from research_and_analyst.vectorstore import InterviewSectionStore
from research_and_analyst.evaluation import SectionEvalCollector
...
class InterviewGraphBuilder:
    ...
    def __init__(self, llm, tavily_search, embeddings=None):
        """
        Initialize the InterviewGraphBuilder with the LLM model and Tavily search tool.

        Args:
            llm: chat model used for question/answer/section generation.
            tavily_search: Tavily search tool instance.
            embeddings: optional embeddings model, required to persist
                interview sections to Pinecone. If omitted, Pinecone storage
                is silently skipped.
        """
        self.llm = llm
        ...
        # Redis-backed search cache (falls back to in-memory if REDIS_URL unset/unreachable)
        self.cache = SearchResultCache(namespace="tavily_search")

        # Presidio PII redaction, applied to search result content before it
        # enters the interview context / prompts
        self.redactor = get_redactor()

        # Pinecone storage for completed interview sections (no-op if
        # PINECONE_API_KEY / embeddings unavailable)
        self.section_store = InterviewSectionStore(embeddings) if embeddings is not None else None

        # Accumulates (question, answer, contexts) triples for RAGAS scoring
        # after the report completes
        self.eval_collector = SectionEvalCollector()
    ...
    def _search_web(self, state: InterviewState):
        ...
        try:
            ...
            #  CACHE CHECK (Redis-backed, falls back to in-memory per-process cache)
            cached_docs = self.cache.get(query)
            if cached_docs is not None:
                self.logger.info(
                    "Returning cached Tavily results", query=query, cache_backend=self.cache.backend
                )
                search_docs = cached_docs
            else:
                ...
                # Store in cache
                self.cache.set(query, search_docs)

            if not search_docs:
                self.logger.warning("No search results found")
                return {"context": ["[No search results found.]"]}

            # PII REDACTION: scrub any personal data embedded in scraped page
            # content before it becomes part of the interview context / prompt
            redacted_docs, pii_hits = self.redactor.redact_search_documents(search_docs)
            if pii_hits:
                self.logger.info("PII redacted from search results", pii_entity_count=pii_hits)
    ...
    def _write_section(self, state: InterviewState):
            ...
            try:
                ...
                # Persist the section to Pinecone, namespaced by topic (no-op if unavailable)
                if self.section_store is not None:
                    self.section_store.upsert_section(
                        thread_id=topic or "untitled",
                        analyst_name=analyst.name,
                        topic=topic,
                        section_text=section.content,
                    )

                # Record (question, answer, contexts) for a later RAGAS pass.
                # The opening message carries the topic framing that prompted
                # this analyst's line of questioning.
                opening_question = state["messages"][0].content if state.get("messages") else topic
                self.eval_collector.add(
