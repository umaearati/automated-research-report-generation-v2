...
from typing import Optional  # noqa: F401 (re-declared for clarity; used below)
...
from research_and_analyst.observability import configure_langsmith, get_tracing_callbacks
from research_and_analyst.evaluation import evaluate_sections
...
class AutonomousReportGenerator:
    ...
    def __init__(self, llm, embeddings=None):
        ...
        if not tavily_key:
            raise ResearchAnalystException(
                "TAVILY_API_KEY is not set in the environment", ValueError("Missing TAVILY_API_KEY")
            )
        ...
        self.logger = logging.LoggerAdapter(log, {"component": "AutonomousReportGenerator"})
    ...
    def create_analyst(self, state: GenerateAnalystsState):
        ...
        try:
            self.logger.info("Starting report generation pipeline | topic=%s", topic)
            ...
            self.logger.info("Analysts created | count=%s", len(analysts.analysts))
            return {"analysts": analysts.analysts}
        except Exception as e:
            self.logger.error("Error creating analysts | error=%s", str(e), exc_info=True)
    ...
    def human_feedback(self):
        ...
        try:
            ...
            self.logger.error("Error during feedback stage | error=%s", str(e), exc_info=True)
    ...
    def write_report(self, state: ResearchGraphState):
        ...
        try:
            ...
            self.logger.info("Writing report | topic=%s", topic)
            ...
            self.logger.error("Error writing main report | error=%s", str(e), exc_info=True)
    ...
    def write_introduction(self, state: ResearchGraphState):
        ...
        try:
            ...
            self.logger.info("Generating introduction | topic=%s", topic)
            ...
            self.logger.info("Introduction generated | length=%s", len(intro.content))
            return {"introduction": intro.content}
        except Exception as e:
            self.logger.error("Error generating introduction | error=%s", str(e), exc_info=True)
    ...
    def write_conclusion(self, state: ResearchGraphState):
        ...
        try:
            ...
            topic = state["topic"]
            formatted_str_sections = "\n\n".join([f"{s}" for s in sections])
            self.logger.info("Generating conclusion | topic=%s", topic)
            system_prompt = INTRO_CONCLUSION_INSTRUCTIONS.render(
                topic=topic, formatted_str_sections=formatted_str_sections
            )
            ...
            self.logger.info("Conclusion generated | length=%s", len(conclusion.content))
            return {"conclusion": conclusion.content}
        except Exception as e:
            self.logger.error("Error generating conclusion | error=%s", str(e), exc_info=True)
    ...
    def finalize_report(self, state: ResearchGraphState):
        ...
        try:
            ...
            self.logger.error("Error finalizing report | error=%s", str(e), exc_info=True)
    ...
    def save_report(self, final_report: str, topic: str,
        ...
        try:
            self.logger.info("Saving report | topic=%s | format=%s", topic, format)
            ...
            self.logger.info("Report saved successfully | path=%s", file_path)
            return file_path

        except Exception as e:
            self.logger.error("Error saving report | error=%s", str(e), exc_info=True)
    ...
    def _save_as_docx(self, text: str, file_path: str):
        ...
        try:
            ...
            self.logger.error("DOCX save failed | path=%s | error=%s", file_path, str(e), exc_info=True)
    ...
    def _save_as_pdf(self, text: str, file_path: str):
        ...
        try:
            ...
            c.save()
            self.logger.info("Centered PDF saved successfully | path=%s", file_path)


        except Exception as e:
            self.logger.error("PDF save failed | path=%s | error=%s", file_path, str(e), exc_info=True)
            raise ResearchAnalystException("Error saving PDF report", e)

    # ----------------------------------------------------------------------
    def evaluate_report_quality(self) -> Optional[dict]:
        """
        Run RAGAS evaluation (faithfulness, answer_relevancy, context_precision)
        over the interview sections accumulated during the most recent report
        run. Call after a report finishes; returns None if no sections were
        recorded or RAGAS isn't installed.
        """
        interview_builder = getattr(self, "interview_builder", None)
        if interview_builder is None:
            self.logger.warning("No interview builder found — call build_graph() first")
            return None

        records = interview_builder.eval_collector.records
        if not records:
            self.logger.info("No interview sections recorded for evaluation")
            return None

        scores = evaluate_sections(records, llm=self.llm, embeddings=self.embeddings)
        if scores:
            self.logger.info("RAGAS evaluation scores | scores=%s", scores)
        return scores
    ...
    def build_graph(self):
        ...
        try:
            ...
            self.interview_builder = InterviewGraphBuilder(
                self.llm, self.tavily_search, embeddings=self.embeddings
            )
            interview_graph = self.interview_builder.build()
            ...
            self.logger.error("Error building report graph | error=%s", str(e), exc_info=True)
...
if __name__ == "__main__":
    try:
        ...
        thread = {"configurable": {"thread_id": "1"}, "callbacks": reporter.tracing_callbacks}
        reporter.logger.info("Starting report generation pipeline | topic=%s", topic)
        ...
        if final_report:
            ...
            reporter.evaluate_report_quality()
        else:
            reporter.logger.error("No report content generated")

    except Exception as e:
        # log.error("Fatal error in main execution", error=str(e))
        log.error("Fatal error in main execution | error=%s", str(e), exc_info=True)
